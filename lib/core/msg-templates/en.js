...
🔒 LPLock: *${parseInt(pair.extra_info.lp_lock_p * 100)}%* | TokenLock: *${parseInt(pair.extra_info.token_lock_p * 100)}%*`
    ...
    static extra_info(pair) {
        return `
[  ](https://t.me/GDOG_SuperBot/app?startapp=tokendetail-${pair.token_contract})
\`${pair.token_contract}\`
`
    }
...
🔹 ${pair.twitter_related.retweet_count} ReTweet | ${pair.twitter_related.favorite_count} Favorite`
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{
                        ...
                        "text": "🐣Details",
                        "url": `https://t.me/GDOG_SuperBot/app?startapp=tokendetail-${pair.token_contract}`
                    }]
                ]
            }
        }
    ...
    static constract_summary(data) {
        for (let i in data) {
            data[i].live_p = parseInt(data[i].live_count / data[i].total_count * 100) + "%"
            for (let k in data[i]) {
                data[i][k] = data[i][k].toString().padStart(3);
            }
        }
...
│ Live │ ${data[0].live_p}   ${data[1].live_p}   ${data[2].live_p} │
└──────┴─────────────────┘
\`\`\`
`,
            options: {
    ...
    static constract_hot(data) {
        let msgs = [];
        for (let i in data) {
            msgs.push(`🔹 [${data[i].symbol}](https://www.dextools.io/app/en/ether/pair-explorer/${data[i].pair_contract})`);
        }
...
`,
    ...
    static constract_my(data) {
        return {
            message: `👤 Profile 👤

🔹 Plan: ${this.VIPPlan[data.vip_level]}
🔹 Credits: ${data.credits}
🔹 TodayGain: ${data.today_credits}
🔹 UsageLimit: ${data.push_balance}
🔹 RateLimit: ${data.msg_cd >= 60 * 1000 ? parseInt(data.msg_cd / (60 * 1000)) + ' min' : parseInt(data.msg_cd / 1000) + " sec"}
`,
            options: {
                parse_mode: "MarkDown",
            }
...
🔹 UsageLimit: ${data.push_balance}
🔹 RateLimit: ${data.msg_cd >= 60 * 1000 ? parseInt(data.msg_cd / (60 * 1000)) + ' min' : parseInt(data.msg_cd / 1000) + " sec"}
🔸 To increase the rate limit, please upgrade the user level.
`,
            options: {
                parse_mode: "MarkDown",
            }
