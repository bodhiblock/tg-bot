...
class TGBot {
        ...
        while(true) {
            try {
                ...
                if (response.data.ok && response.data.result && response.data.result.length > 0) {
                    await this.onMessageUpdate(response.data.result);
                }
            } catch(error) {
                plog.error(`TG MSGLOOP ${JSON.stringify(error)}`);
            } finally {
    ...
    async onMessageUpdate(data) {
        for (let k in data) {
            await this.onMessage(data[k].message);
            this.tgbot_update_offset = data[k].update_id + 1;
        }       
    }
    ...
    async sendMessage(chatId, msg) {
        try {
            ...
            if (response && response.data && response.data.ok) {
                ...
                plog.error(`TG SENDMSG1 ${response.toString()}`);
            }
        } catch (error) {
            plog.error(`TG SENDMSG2 ${error.toString()}`);
        }
    }
}
